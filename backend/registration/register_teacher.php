...
foreach ($extraColumns as $colDef) {
    ...
    $check = $conn->query("SHOW COLUMNS FROM teachers LIKE '" . $conn->real_escape_string($colName) . "'");
    if ($check && $check->num_rows === 0) {
        $conn->query("ALTER TABLE teachers ADD COLUMN $colDef");
    }
}
...
$passwordPlain = random_alnum_t(5);
